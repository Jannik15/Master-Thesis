...
public class JTestScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.M)) // Change shader on geometry
        {
            ...
            Debug.Log("Attempted to swap shader for " + materialObjects[roomIndex].Count + " materials in room " + roomIndex + " with the shader " + materialContainer.shaders[materialIndex]);
        }
